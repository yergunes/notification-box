...
informations=$(cat $BASE/moduls/.wordpress-config);
I=$IFS; IFS="" 
...
if [ "$update_count" != "0" ] && [ "$comment_count" != "0" ]; then
    notify-send -i $BASE/wordpress-icon.png "Wordpress" "You got $update_count new update and $comment_count new comment"
elif [ "$update_count" != "0" ] && [ "$update_count" != "" ]; then
    notify-send -i $BASE/wordpress-icon.png "Wordpress" "You got $update_count new update"
elif [ "$comment_count" != "0" ] && [ "$comment_count" != "" ]; then
    notify-send -i $BASE/wordpress-icon.png "Wordpress" "You got $comment_count new comment"
fi
