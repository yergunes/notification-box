...
if [ "$auth" != "0" ]; then
    notify-send -i $BASE/icons/gmail-notification-icon.png "Gmail" "Authorization error! Check your account settings...";
else
    while [ 1 ]; do
        ...
        if [ "$mail_count" != "0" ]; then
            notify-send -i $BASE/icons/gmail-notification-icon.png "Gmail" "You got $mail_count new email!";
        fi
